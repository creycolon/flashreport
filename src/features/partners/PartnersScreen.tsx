...
export const PartnersScreen = () => {
    ...
    if (isWebDesktop) {
        ...
        const handleSaveEnhanced = async (partner: any) => {
            const payload = {
                name: partner.name,
                alias: partner.alias,
                participationPercentage: partner.participation_percentage,
                role: partner.role,
                isManagingPartner: partner.is_managing_partner ? 1 : 0,
                isActive: partner.is_active
            };

            try {
                if (partner.id) {
                    await (partnerRepository as any).update(partner.id, payload);
                } else {
                    await (partnerRepository as any).create(payload);
                }
                loadData();
        ...
        const transformedPartners = partners.map(item => ({
            id: item.id,
            name: item.name,
            alias: item.alias || '',
            participation_percentage: item.participation_percentage || 0,
            role: item.role,
            is_managing_partner: item.is_managing_partner === true,
